...
export default function SupportChatPage() {
  ...
  const chatSummaries = useMemo(() => {
    const map = new Map<
      string,
      { lastMsg: string; lastTime: number; count: number }
    >();
  ...
  const handleEndSupport = async () => {
    if (!activePatientId) return;
    try {
      await update(ref(db, `users/${activePatientId}`), {
        needsSupport: false,
      });
  ...
  const handleClearChat = async () => {
    ...
    if (
      !confirm(
        "Clear entire chat history for this patient? This cannot be undone.",
      )
    )
      return;
    try {
      ...
      const q = query(
        chatRef,
        orderByChild("userId"),
        equalTo(activePatientId),
      );
      const { get } = await import("firebase/database");
      const snap = await get(q);
      const updates: Record<string, null> = {};
      snap.forEach((child) => {
        updates[`chat_messages/${child.key}`] = null;
      });
  ...
  return (
    <div
      className="flex bg-[#141A14] card-style overflow-hidden"
      style={{ height: "calc(100vh - 120px)" }}
    >
      {/* LEFT PANEL — Patient List */}
      <div
        className={`w-full md:w-80 border-r border-[rgba(212,184,150,0.08)] bg-[#1A2620] flex flex-col ${showMobileChat ? "hidden md:flex" : "flex"}`}
      >
            ...
            return (
              ...
              >
                <div className="relative shrink-0">
                  ...
                  {p.needsSupport && (
                    <div className="absolute bottom-0 right-0 w-3 h-3 bg-[#E05252] rounded-full border-2 border-[#1A2620] animate-pulse"></div>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <h4
                      className={`text-sm font-semibold truncate ${activePatientId === p.userId ? "text-[#D4B896]" : "text-[#F0E6D3]"}`}
                    >
                      {p.name || "Unknown"}
                    </h4>
                    {summary?.lastTime ? (
                  ...
                  {summary?.lastMsg ? (
                    <p className="text-xs text-[#7A8A76] truncate mt-0.5">
                      {summary.lastMsg}
                    </p>
                  ) : (
                    <p className="text-xs text-[#5C6B58] italic mt-0.5">
                      No messages yet
                    </p>
                  )}
                  {/* Status badges */}
          ...
          {filteredPatients.length === 0 && (
            <div className="p-6 text-center space-y-3">
              ...
              {activeTab === "open" && (
                <button
                  onClick={() => setActiveTab("all")}
                  className="text-[#D4B896] text-xs underline hover:text-[#C4A882]"
                >
                  View all patients
                </button>
              )}
      ...
      {/* RIGHT PANEL — Chat */}
      <div
        className={`flex-1 flex flex-col bg-[#111811] ${showMobileChat ? "flex" : "hidden md:flex"}`}
      >
        {activePatient ? (
          <>
            {/* Chat Header */}
            <div className="h-16 border-b border-[rgba(212,184,150,0.1)] bg-[#1C2B1E] flex items-center justify-between px-4 md:px-6 shrink-0">
              <div className="flex items-center space-x-3 md:space-x-4">
                ...
                <button
                  ...
                  className="md:hidden p-1.5 hover:bg-[rgba(212,184,150,0.1)] rounded-lg transition-colors"
                >
                ...
                <div>
                  <h4 className="font-poppins font-medium text-[#F0E6D3] truncate">
              ...
              <div className="flex items-center space-x-2 md:space-x-3">
                ...
                <a
                  ...
                  className="p-2 bg-[rgba(212,184,150,0.1)] hover:bg-[rgba(212,184,150,0.2)] text-[#D4B896] rounded-lg transition-colors"
                >
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-3 md:space-y-4">
              {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-[#7A8A76] space-y-3">
                  ...
                  <p className="text-xs text-[#5C6B58] max-w-xs text-center">
                    The patient hasn&apos;t sent any messages. You can start the
                ...
                messages.map((m) => {
                  ...
                  if (isSystem) {
                    ...
                    const mapsUrl = m.text.match(
                      /(https:\/\/www\.google\.com\/maps\?q=[^\s]+)/,
                    )?.[1];
                    const isEmergencyLoc =
                      m.text.includes("EMERGENCY LOCATION");
                    ...
                    if (isLocation && mapsUrl) {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`${isEmergencyLoc ? "bg-[#2D1515] border-[#E05252]/30" : "bg-[#1C2B1E] border-[#4CAF78]/30"} border rounded-xl px-4 py-3 max-w-sm w-full`}
                          >
                            <div className="flex items-center gap-2 mb-2">
                              <div
                                className={`p-1.5 rounded-lg ${isEmergencyLoc ? "bg-[#E05252]/15" : "bg-[#4CAF78]/15"}`}
                              >
                                <svg
                              ...
                              <span
                                className={`text-[10px] font-bold uppercase tracking-wider ${isEmergencyLoc ? "text-[#E05252]" : "text-[#4CAF78]"}`}
                              >
                                {isEmergencyLoc
                                  ? "🚨 Emergency Location"
                                  : "📍 Patient Location"}
                            ...
                            <p className="text-[#9BA897] text-xs font-mono mb-2">
                              {lat}, {lng}
                            </p>
                            <a
                              href={mapsUrl}
                            ...
                            <p className="text-[9px] text-[#5C6B58] mt-1.5 text-right">
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`max-w-[85%] md:max-w-[70%] rounded-2xl px-4 md:px-5 py-2.5 md:py-3 shadow-md ${
                          isSupport
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <span
                            className={`text-[10px] font-bold uppercase tracking-wider ${isSupport ? "text-[#4CAF78]" : "text-[#D4B896]"}`}
                          >
                        ...
                        {m.mediaUrl && (
                          ...
                          >
                            <img
                              src={m.mediaUrl}
                              alt="Shared media"
                              className="max-w-full max-h-60 object-contain rounded-lg transition-transform group-hover:scale-[1.02]"
                              loading="lazy"
                            />
                        ...
                        <div
                          className={`text-[9px] md:text-[10px] mt-1.5 text-right ${isSupport ? "text-[#9BA897]" : "text-[#7A8A76]"}`}
                        >
                          {format(m.timestamp, "h:mm a")}
                        </div>
                      </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
            ...
            <h2 className="font-poppins font-semibold text-xl md:text-2xl text-[#F0E6D3] mb-2">
              Guardian Support
            </h2>
            <p className="text-[#9BA897] max-w-sm text-sm">
              Select a patient from the sidebar to view their chat history or
            ...
            {openCount > 0 && (
              <div className="mt-4 bg-[rgba(224,82,82,0.1)] border border-[rgba(224,82,82,0.3)] px-4 py-2 rounded-xl">
                <p className="text-[#E05252] text-sm font-medium">
                  ⚠ {openCount} patient{openCount > 1 ? "s" : ""}{" "}
                  {openCount > 1 ? "need" : "needs"} support
                </p>
              </div>
            )}
          </div>
