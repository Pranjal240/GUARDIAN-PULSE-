...
import { useAllPatients, useLiveVitals, useMonitorStatus } from "@/lib/firebase-hooks";
  ...
  return (
    <motion.div
      ...
      className={`card p-4 transition-shadow cursor-pointer ${
        status === "disabled"
          ? "opacity-70 border border-[#E05252]/30"
          : status === "critical"
            ? "animate-critical border border-[#E05252]/40"
            : ""
      }`}
    >
      {/* Header */}
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="font-poppins font-semibold text-[#F0E6D3] truncate">
            {patient?.name ?? "Unknown"}
          </h4>
          <p className="text-xs text-[#7A8A76] truncate">
            {patient?.role ?? "patient"}
          </p>
          <p className="text-xs text-[#6B7F67] capitalize">
            {patient?.mode ?? "normal"} mode
          </p>
        </div>
        <span
      ...
      {isCalculating ? (
        ...
        <EcgChart data={ecgData.slice(-40)} height={90} status={status as "normal" | "warning" | "critical"} />
      )}
