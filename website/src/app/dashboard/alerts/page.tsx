...
import { useAllAlerts, useAllUsers } from "@/lib/firebase-hooks";
...
export default function AlertsPage() {
  ...
  const { data: allUsers, loading: pLoading } = useAllUsers();
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="font-poppins font-bold text-3xl text-[#F0E6D3] mb-2">
            Alert Management
          </h1>
          <p className="text-[#9BA897]">
            Monitor, manage, and resolve critical incidents.
          </p>
        </div>
      </div>
      ...
      <Tabs.Root defaultValue="active" className="flex flex-col h-full">
        <Tabs.List className="flex space-x-2 mb-6">
          ...
          >
            ...
            <span className="ml-2 opacity-50 font-mono-data">
          ...
          <AnimatePresence>
            {activeAlerts.length === 0 ? (
              ...
              >
                ...
                <h3 className="font-poppins font-semibold text-2xl text-[#F0E6D3] mb-2">
                  All Clear
                </h3>
                <p className="text-[#9BA897]">
                  There are no active alerts requiring attention.
                </p>
              </motion.div>
            ) : (
              activeAlerts.map((alert) => (
                <ActiveAlertCard
                  ...
                  alert={alert}
                  patient={(() => {
                    const exact = allUsers.find((p) => p.userId === alert.userId);
                    return exact?.email 
                      ? (allUsers.find((p) => p.email === exact.email && !!p.phone) || exact)
                      : exact;
                  })()}
                />
              ))
