...
import {
  ...
  classifyAlertType,
  calculateBpmStatus,
  usePendingAdminCount,
  useSupportRequestCount,
} from "@/lib/firebase-hooks";
import { migrateCardiacToSOS } from "@/lib/demo-data-seeder";
...
export default function DashboardOverview() {
  ...
  // Run one-time migration to fix old cardiac→SOS alerts
  useEffect(() => {
    migrateCardiacToSOS();
  }, []);
    ...
    <div className="space-y-6">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h4 className="font-poppins font-semibold text-[#F0E6D3]">
                      {pendingAdminCount} Pending Admin Request
                      {pendingAdminCount > 1 ? "s" : ""}
                    </h4>
                    <p className="text-[#7A8A76] text-sm">
                      Review and approve new admin access requests
                    </p>
                  </div>
                </div>
      ...
      >
        ...
        <StatCard
          ...
          iconColorClass={
            stats.activeAlerts > 0 ? "text-[#E05252]" : "text-[#7A8A76]"
          }
        />
        <StatCard
        ...
        <StatCard
          ...
          iconColorClass={
            stats.criticalToday > 0 ? "text-[#D4943A]" : "text-[#7A8A76]"
          }
        />
      </motion.div>
        ...
        {[
          {
            ...
            label: "Support Chat",
            desc: `${supportCount} open requests`,
            icon: MessageSquare,
            color: supportCount > 0 ? "#E05252" : "#D4B896",
          },
          {
            href: "/dashboard/analytics",
            label: "Analytics Hub",
            desc: "Charts, trends & metrics",
            icon: BarChart3,
            color: "#D4B896",
          },
          {
          ...
          },
          {
            ...
            desc: `${pendingAdminCount} pending`,
            icon: ShieldCheck,
            color: "#4CAF78",
          },
        ].map((card, i) => (
            ...
            <Link href={card.href}>
              <div className="card-glass p-4 flex items-center space-x-4 hover-float group">
                ...
                <div className="flex-1">
                  <h4 className="font-poppins font-semibold text-[#F0E6D3] text-sm">
                    {card.label}
                  </h4>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="flex items-center space-x-3 mb-2">
            <h2 className="font-poppins font-semibold text-xl text-[#F0E6D3]">
              Live Alerts
            </h2>
          ...
          <AnimatePresence>
            {activeAlerts.length === 0 ? (
              ...
              >
                ...
                <h3 className="font-poppins font-semibold text-xl text-[#F0E6D3] mb-1">
                  All Clear
                </h3>
              ...
              activeAlerts.map((alert) => {
                const pInfo = allUsers.find((p) => p.userId === alert.userId);
                ...
                return (
                  ...
                  >
                    <div className="flex items-center space-x-4 flex-1 mb-4 sm:mb-0">
                      <div
                        className="h-12 w-12 rounded-full flex items-center justify-center font-poppins font-bold text-lg"
                        style={{
                      ...
                      <div>
                        <h4 className="font-poppins font-medium text-[#F0E6D3]">
                          {pInfo?.name ?? "Unknown"}
                        </h4>
                        <span className="text-xs text-[#9BA897]">
                    ...
                    <div className="flex items-center space-x-2 w-full sm:w-auto mt-4 sm:mt-0 justify-end">
                      ...
                      {pInfo?.phone && (
                        <a
                          href={`tel:${pInfo?.phone ?? ""}`}
                          className="bg-[rgba(212,184,150,0.1)] hover:bg-[rgba(212,184,150,0.2)] text-[#D4B896] p-2 rounded-lg transition-colors"
                        >
                          <Phone className="h-4 w-4" />
                        </a>
                      )}
                      <a
                        href={`/dashboard/ecg?userId=${alert.userId}`}
                        className="border border-[rgba(212,184,150,0.3)] hover:border-[#D4B896] text-[#D4B896] p-2 rounded-lg transition-colors"
                      >
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {patientIds.map((id) => {
              ...
              const isOff = p?.monitorDisabled === true;
              const data = latestEcgMap.get(id) || [];
              const currentBpm = isOff
                ? "OFF"
                : data.length > 0 ? data[data.length - 1].bpm : "--";
              const sColor = isOff
                ? "#E05252"
                : currentBpm === "--"
                  ? "#9BA897"
                  : calculateBpmStatus(currentBpm as number) === "normal"
                    ? "#4CAF78"
                    : calculateBpmStatus(currentBpm as number) === "warning"
                      ? "#D4943A"
                      : "#E05252";
              ...
              return (
                <motion.div
                  ...
                  className={`card-glass p-3 flex flex-col justify-between h-28 cursor-pointer relative overflow-hidden group ${isOff ? 'opacity-60' : ''}`}
                >
                    ...
                    {isOff ? (
                      <div className="w-full h-full flex items-center justify-center">
                        <svg viewBox="0 0 200 60" preserveAspectRatio="none" className="w-full h-full">
                          <line x1="0" y1="30" x2="200" y2="30" stroke="#E05252" strokeWidth="1.5" opacity="0.5" />
                        </svg>
                      </div>
                    ) : (
                    <div
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
        ...
        >
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-poppins font-medium text-[#F0E6D3]">
              Alerts — Last 7 Days
            </h3>
          </div>
          <div className="h-64">
          ...
          <div className="h-64 flex relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none pr-32">
              <div className="text-center">
                ...
                <span className="text-[10px] text-[#7A8A76] uppercase tracking-wider">
                  Total
                </span>
              </div>
            </div>
